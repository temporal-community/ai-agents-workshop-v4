...
| 3 | **Multi-agent** | A triage agent routes to two specialists, each a Child Workflow on its own Task Queue — and fans out across several at once with `Promise.all`. |
| 4 | **Heterogeneous agents** | The travel specialist is now **Python** (LangChain Deep Agents), started as an ordinary Child Workflow by type name. The TypeScript orchestrator does not change. |
...
# Challenge 3 — two specialists and a parallel fan-out
npm run c3:worker
...
- **Challenge 3** — one `StartChildWorkflowExecution` per specialist, and one more per city when
  the fan-out tool runs. Each specialist is its own Workflow Execution on the specialist Task Queue.
- **Challenge 4** — the orchestrator's history is unchanged from challenge 3, but the Workflow
  behind the travel call is a Python one, with its own model and tool Activities inside it.
...
├── python-travel-planner/            Python Deep Agents specialist for challenge 4
└── src/
    ├── shared/
...
- [Temporal + LangChain Deep Agents (Python)](https://docs.temporal.io/develop/python/integrations/deepagents)
