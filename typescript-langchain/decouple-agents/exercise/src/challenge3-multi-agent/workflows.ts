...
export async function triageAgentWorkflow(request: AgentRequest): Promise<string> {
  ...
  // TODO 6: Replace this stub with a `childWorkflowAsTool({ ... })` call, the
  ...
  const travelTool = tool({
    ...
    execute: async ({ question }) => {
      throw new Error(`TODO 6: reach the travel specialist as a Child Workflow (${question})`);
    },
  });

  // TODO 7: Make this fan out. Start one Child Workflow per city and run them
  ...
  const compareTool = tool({
    ...
    execute: async ({ cities }) => {
      throw new Error(`TODO 7: fan out across ${cities.length} cities with Promise.all`);
    },
  });
