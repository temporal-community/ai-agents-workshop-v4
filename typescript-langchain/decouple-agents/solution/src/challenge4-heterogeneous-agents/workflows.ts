...
export async function triageAgentWorkflow(request: AgentRequest): Promise<string> {
  ...
  // The same Child Workflow call as challenge 3, with two strings changed.
  //
  ...
  const travelTool = tool({
    ...
    execute: async ({ question }) => {
      const response = await wf.executeChild<PythonTravelPlannerWorkflow>(PYTHON_TRAVEL_PLANNER_WORKFLOW, {
