...
// The Child Workflow counterpart of activityAsTool / nexusOperationAsTool, which the SDK ships but this does not.
...
export function childWorkflowAsTool(options: ChildWorkflowAsToolOptions): Tool {
  return tool({
    ...
    execute: async ({ question }) => {
      // TODO 8: Start the specialist with `wf.executeChild(options.workflow, …)`
      ...
      throw new Error(`TODO 8: run ${options.workflow.name} as a Child Workflow (${question})`);
    },
  });
}
