...
| 10 | `src/challenge3-multi-agent/handler.ts` | The `WorkflowRunOperationHandler` behind the Nexus Operation. |
| 11 | `src/challenge3-multi-agent/workflows.ts` | Build the travel tool with `nexusOperationAsTool`. |
...
| # | File | What you write |
|---|---|---|
| 13 | `src/challenge4-heterogeneous-agents/api.ts` | Declare the operation matching Python's `ask_travel_planner`. |
| 14 | `src/challenge4-heterogeneous-agents/workflows.ts` | Point the travel tool at the Nexus Operation. |
| 15 | `src/challenge4-heterogeneous-agents/client.ts` | Create the endpoint targeting the Python Worker's Task Queue. |
