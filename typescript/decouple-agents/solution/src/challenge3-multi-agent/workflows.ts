...
export async function triageAgentWorkflow(request: AgentRequest): Promise<string> {
  ...
  // Nexus: the orchestrator names an endpoint and an operation. It does not
  // know the Workflow type, the Task Queue, or even the Namespace behind it —
  // which is exactly what lets challenge 4 swap the implementation language.
  const travelTool = nexusOperationAsTool(
    travelPlannerService.operations.askTravelPlanner,
    {
      ...
      parameters: {
        type: 'object',
        properties: { question: { type: 'string', description: 'The travel question, in plain English' } },
        required: ['question'],
    ...
    },
    {
      ...
      endpoint: TRAVEL_PLANNER_ENDPOINT,
