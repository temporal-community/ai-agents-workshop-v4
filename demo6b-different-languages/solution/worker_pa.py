# ABOUTME: Personal-assistant team's worker — orchestrator + weather agent.
# The travel planner is a Java worker (Nexus), so it's NOT registered here — only weather + orchestrator.
...
async def main() -> None:
    ...
    orchestrator_worker = Worker(
        ...
        # No travel-planner activity here — that specialist is a Java Spring AI
        # worker reached over Nexus (endpoint "travel-planner"). The orchestrator
        # only needs the Nexus caller wiring, which the plugin handles.
    )
